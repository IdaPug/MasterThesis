...
class FilteredTotalSegmentatorV2Dataset(Dataset):
    ...
    def __init__(
        ...
        image_dtype: torch.dtype = torch.float32,
        label_dtype: torch.dtype = torch.long,
        subjects: Optional[List[str]] = None,   # ✅ filtering support
    ):
        self.root_dir = root_dir
        self.group_map = group_map
        self.transforms = transforms
        ...
        if subjects is not None:
            # Normalize user-provided list (accepts 's001' or full paths)
...
class CachedSliceDataset(Dataset):
    ...
    def __init__(
        ...
        # Precompute (subject_idx, slice_idx) pairs — without loading full volumes
        self.index = []
        for subj_idx, subj_dir in enumerate(base_dataset.subjects):
            ct_path = subj_dir + "/ct.nii.gz"
            shape = nib.load(ct_path).shape  # fast: just header read
            if plane == "axial":
                num_slices = shape[0]
            elif plane == "coronal":
                num_slices = shape[1]
...
class VolumeSliceDataset(Dataset):
    ...
    def __init__(
        ...
        slice_transform=None,
        mask_transform=None,
        slice_step: int = 1,      # NEW: spacing between slices (1 = all slices)
    ):
        """
        Args:
        ...
        self.slice_transform = slice_transform
        self.mask_transform = mask_transform
        self.slice_step = max(1, int(slice_step))  # safety
...
class DinoSubjectDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "features": features.float(),  # cast back to float32 for training
            "masks": masks.long(),
            "meta_list": meta_list
        }
...
class FilteredDINOFFeatureDataset(Dataset):
    ...
    def __init__(self, root_dir: str, plane: str = "axial", subject_ids: list[str] = None):
        ...
        # print number of subjects and slices found
        #unique_subjects = {os.path.basename(os.path.dirname(os.path.dirname(os.path.dirname(f)))) for f in self.files}  
        #print(f"FilteredDINOFFeatureDataset: {len(self.files)} files loaded from {root_dir} for subjects: {unique_subjects}")
...
class FilteredDINOFeatureAndSkipDataset(Dataset):
    ...
    # Build file list for DINO features
        ...
        if len(missing_skips) > 0:
            print(f"⚠️ Warning: {len(missing_skips)} slices missing skip features.")
...
class FilteredDINOFeatureAndSkipDataset2(Dataset):
    ...
    def __init__(self, root_dir: str, plane: str = "axial", subject_ids: list[str] = None):
        ...
        if len(missing_skips) > 0:
            print(f"⚠️ Warning: {len(missing_skips)} slices missing main skip features.")
