...
from Decoder import UnetDecoder, AdapterUnetDecoder, DINOv3UNetDecoder, VanillaUNet
...
def main():
    parser = argparse.ArgumentParser(description="Vanilla UNet testing with per-subject fusion")
    ...
    parser.add_argument('--TestSize', type=int, default=100, help='Number of subjects to use for testing')
    parser.add_argument('--BatchSize', type=int, default=16, help='Batch size used during training')
    ...
    #model_path = f"../{args.Model_path}/TrainSize{args.TrainSizePercent}/best_model.pth"
    model_path = f"../{args.Model_path}/TrainSize{args.TrainSize}/BatchSize{args.BatchSize}/best_model.pth"
    ...
    # output dirs
    savedir = os.path.join("../3DSegmentationCurated", f"VanillaUNet_model/TrainSize{args.TrainSizePercent}")
    os.makedirs(savedir, exist_ok=True)
    savedir_plot = os.path.join("../predictions_skipCurated", f"VanillaUNet_model/TrainSize{args.TrainSizePercent}")
    ...
    LRType = 'constant'
    # save to txt file
    plottingmodel = "VanillaUNetALT" + "_" + LRType
