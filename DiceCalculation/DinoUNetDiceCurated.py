...
def main():
    ...
    parser.add_argument("--DinoModel", type=str, default="dinov3_vits16", help="DINOv3 model variant to use")
    ...
    parser.add_argument('--TrainSize', type=int, default=150, help='Number of subjects used during training')
    parser.add_argument('--DataSamp', type=str, default='Curated', help='Data sampling strategy during training')
    ...
    print(f"Loaded {dinomodel} on {device}")
    ...
    model_path = f"../{args.Model_path}/dinomodel_{dinomodel}/TrainSize{args.TrainSize}/BatchSize{args.BatchSize}/best_model.pth"
    ...
    # output dirs
    savedir = os.path.join(f"../3DSegmentation", f"model_{args.model}/TrainSize{args.TrainSize}_Data{args.DataSamp}_Batch{args.BatchSize}")
    print(savedir)
    os.makedirs(savedir, exist_ok=True)
    savedir_plot = os.path.join("../predictions",  f"model_{args.model}/TrainSize{args.TrainSize}_Data{args.DataSamp}_Batch{args.BatchSize}")
    ...
    modelname =  args.model + "_" + dinomodel 

    # for 1 row
    row_dict = {"model": modelname, "TrainSize": args.TrainSize}
    ...
    # save to txt file
    plottingmodel = dinomodel + "_" + args.model + "_" + args.DataSamp + f"_{args.LRFactor}_{args.LRPatience}"
    file_dir = "../ThesisPlotting/DinoModelsDice_AttnModelsCurated.txt"
    df = pd.read_csv(file_dir) if os.path.exists(file_dir) else pd.DataFrame(columns=["model", "TrainSize", "DataSamp", "dice"])


    print(plottingmodel, args.TrainSize, overall_mean_dice)
    # print number of columns and rows
    print(f"DataFrame shape: {df.shape}")

    # Check if this model+percent already exists
    mask = (df['model'] == plottingmodel) & (df['TrainSize'] == args.TrainSize  )
    if mask.any():
        print("Entry exists, updating dice score.")
        df.loc[mask, 'dice'] = overall_mean_dice
    else:
        df = pd.concat([df, pd.DataFrame([[plottingmodel, args.TrainSize, args.DataSamp, overall_mean_dice]], columns=df.columns)], ignore_index=True)
